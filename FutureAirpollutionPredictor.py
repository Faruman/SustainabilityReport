    ...
    def predict_airpollution(self, policy_dict:dict= dict()):
        if not self.models_airp:
            ...
            for key in policy_dict.keys():
                filter_columns = [s for s in self.frcst_data.columns if key in s]
                base = self.frcst_data.loc[self.frcst_data.index.min(), filter_columns]
                forecast = base + (base * policy_dict[key])
                slope = (forecast - base) / (self.frcst_data.index.max() - self.frcst_data.index.min())
                for year in list(set(self.frcst_data.index) - set([self.frcst_data.index.min()])):
                    self.frcst_data.loc[year, filter_columns] = base + slope * (year - self.frcst_data.index.min())

                self.frcst_airp = pd.DataFrame(index=self.frcst_data.index, columns=self.pollutants)

                for pollutant in self.pollutants:
                    model = self.models_airp[pollutant]
                    selected_columns = [s for s in self.frcst_data.columns if "yhat_" in s]
                    df_temp = self.frcst_data[selected_columns]
                    ...
                    self.frcst_airp.loc[:, pollutant] = y_pred
